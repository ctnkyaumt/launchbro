...
//  - on an update we only refresh it when CreateShortcutOnUpdate (default on) is set.
// It is never created on a plain launch/update-check.
VOID _app_maybe_create_shortcut (
	_In_ PBROWSER_INFORMATION pbi,
	_In_ BOOLEAN is_firstinstall
)
{
	if (!_r_config_getboolean (L"CreateShortcut", TRUE))
		return;

	if (!is_firstinstall && !_r_config_getboolean (L"CreateShortcutOnUpdate", TRUE))
		return;
...
)
{
	...
	BOOLEAN was_installed;

	pbi = (PBROWSER_INFORMATION)arglist;
	hwnd = _r_app_gethwnd ();

	// snapshot one-shot manual-download request and whether this is a fresh install
	is_forcedownload = pbi->is_forcedownload;
	pbi->is_forcedownload = FALSE;
	was_installed = _r_fs_exists (&pbi->binary_path->sr);
	...
	if (!is_installed && !is_stayopen)
	{
		...
		if (_app_checkupdate (hwnd, pbi, &is_haveerror))
		{
			...
			if ((!is_exists || pbi->is_autodownload || is_forcedownload) && _app_ishaveupdate (pbi))
			{
				if (pbi->is_bringtofront)
					_r_wnd_toggle (hwnd, TRUE); // show window
	...
	{
		// shortcut is created on install/update only (see _app_maybe_create_shortcut),
		// not on every launch. Only keep the registry association fresh here.
	...
	switch (msg)
	{
		...
		case WM_COMMAND:
		{
			...
			switch (ctrl_id)
			{
				...
				case IDC_START_BTN:
				{
					// manual click: user explicitly wants to advance the pipeline
					// (download an already-found update even when the browser is
					// installed and auto-download is off).
					browser_info.is_forcedownload = TRUE;
