...
- fixed the taskbar showing two separate icons (a pinned one and a running one) instead of unifying them when the shortcut was pinned after the browser had already run at least once: every launch now stamps the same AppUserModelID onto the freshly opened window that the shortcut declares, checking first so the property is only touched when it actually differs.
