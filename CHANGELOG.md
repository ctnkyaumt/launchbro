...
- reworked when launchbro opens the browser so its own window is reachable and it never opens the browser unexpectedly: a bare manual launch (no args) now auto-checks for updates AND keeps its window open for you, but does NOT launch the browser at startup or when you close launchbro. The browser is launched only at the very end of an update run, and only when "Run at end" is enabled and an update was actually installed (plus the URL-routing and scheduled-task paths, which pass arguments and behave as before). Day-to-day browsing stays on the Desktop shortcut (direct chrome.exe). ChromiumWaitForDownloadEnd no longer affects browser launching.
- fixed profile import doing nothing (you had to copy manually): it only looked for a source folder under a few exact names, so a folder simply named "profile" (or the profile folder itself, whatever its name) was never found. Import now detects the real profile - whether you pick the profile folder itself (recognized by its Local State / Default / prefs.js contents), a parent holding it under the export name or a generic profile/profileN name, or a parent with a single profile-looking subfolder - and shows a clear message if none is found instead of silently succeeding.
- completed translations for all 16 supported languages: the newer strings (uninstall, export/import profile, scheduled auto-update, app self-update, migration, and related dialogs - 19 to 23 strings per language depending on how much each language already had) were untranslated and fell back to English. All languages are now fully translated for every string in the current UI.
